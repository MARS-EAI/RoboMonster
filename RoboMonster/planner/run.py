# planner/run.py
import multiprocessing as mp
import os
from copy import deepcopy
import time
import argparse
import gymnasium as gym
import numpy as np
import yaml
from tqdm import tqdm
import os.path as osp
from mani_skill.utils.wrappers.record import RecordEpisode
from utils.wrappers.record import RecordEpisodeMA
from utils.wrappers.suction_action import SuctionActionWrapper
from mani_skill.trajectory.merge_trajectory import merge_trajectories
from .solutions import solveCircleVase, solvePlaceBadminton, solveSuctionCard
from .solutions import solvePickPokeball, solveSwipeCard
import re

MP_SOLUTIONS = {
    "SwipeCard-rf": solveSwipeCard,
    "PlaceBadminton-rf": solvePlaceBadminton,
    "CircleVase-rf": solveCircleVase,
    "SuctionCard-rf": solveSuctionCard,
    "PickPokeball-rf": solvePickPokeball
}

def parse_args(args=None):
    parser = argparse.ArgumentParser()
    parser.add_argument("-e", "--env-id", type=str, default="", help=f"Environment to run motion planning solver on. Available options are {list(MP_SOLUTIONS.keys())}")
    parser.add_argument("-c", "--config", type=str, default="configs/robocasa/take_photo.yaml", help='Configuration to build scenes, assets and agents.')
    parser.add_argument("-o", "--obs-mode", type=str, default="none", help="Observation mode to use. Usually this is kept as 'none' as observations are not necesary to be stored, they can be replayed later via the mani_skill.trajectory.replay_trajectory script.")
    parser.add_argument("-n", "--num-traj", type=int, default=10, help="Number of trajectories to generate.")
    parser.add_argument("-s", "--seed", type=int, default=0, help="Seed of the random.")
    parser.add_argument("--only-count-success", action="store_true", help="If true, generates trajectories until num_traj of them are successful and only saves the successful trajectories/videos")
    parser.add_argument("--reward-mode", type=str)
    parser.add_argument("-b", "--sim-backend", type=str, default="cpu", help="Which simulation backend to use. Can be 'auto', 'cpu', 'gpu'")
    parser.add_argument("--render-mode", type=str, default="rgb_array", help="can be 'sensors' or 'rgb_array' which only affect what is saved to videos")
    parser.add_argument("--vis", default=False, action="store_true", help="whether or not to open a GUI to visualize the solution live")
    parser.add_argument("--save-video", action="store_true", help="whether or not to save videos locally")
    parser.add_argument("--traj-name", type=str, help="The name of the trajectory .h5 file that will be created.")
    parser.add_argument("--shader", default="default", type=str, help="Change shader used for rendering. Default is 'default' which is very fast. Can also be 'rt' for ray tracing and generating photo-realistic renders. Can also be 'rt-fast' for a faster but lower quality ray-traced renderer")
    parser.add_argument("--record-dir", type=str, default="demos", help="where to save the recorded trajectories")
    parser.add_argument("--num-procs", type=int, default=1, help="Number of processes to use to help parallelize the trajectory replay process. This uses CPU multiprocessing and only works with the CPU simulation backend at the moment.")
    parser.add_argument("--control-mode", type=str, default="pd_joint_pos", help="The control mode of the robots. Can be 'pd_ee_pose', 'pd_joint_pos'")
    parser.add_argument("--robot", type=str, default='panda', help="The robot used for simulation. Can be 'panda', 'xarm6_robotiq'")
    parser.add_argument("-v", "--variant", type=str, default="gripper",
    choices=["gripper", "ours"],
    help="Choose baseline(gripper-only) or ours heterogeneous multi-end effectors")
    return parser.parse_args()


def _normalize_task_name(raw):
    # "SwipeCard"        -> "swipe_card"
    # "PlaceBadminton"   -> "place_badminton"
    # "pick_pokeball"    -> "pick_pokeball"
    # "swipe_card"       -> "swipe_card"
    if "_" in raw:
        # already snake-ish
        return raw.lower()
    # CamelCase / PascalCase -> snake_case
    s1 = re.sub(r'(.)([A-Z][a-z]+)', r'\1_\2', raw)
    snake = re.sub(r'([a-z0-9])([A-Z])', r'\1_\2', s1).lower()
    return snake

# ======= helpers: safe casting =========
def _as_py_bool(x):
    """ 
    torch.Tensor / np.ndarray / list / bool -> Python bool
    """
    try:
        import torch
        if isinstance(x, torch.Tensor):
            return bool(x.detach().cpu().reshape(-1)[0].item())
    except Exception:
        pass
    if isinstance(x, np.ndarray):
        x = x.reshape(-1)[0]
        try:
            return bool(x.item())
        except Exception:
            return bool(x)
    if isinstance(x, (list, tuple)):
        return bool(x[0])
    return bool(x)

def _as_py_int(x, default=0):
    """ 
    torch.Tensor / np.ndarray / list / int -> Python int
    """
    try:
        import torch
        if isinstance(x, torch.Tensor):
            return int(x.detach().cpu().reshape(-1)[0].item())
    except Exception:
        pass
    if isinstance(x, np.ndarray):
        x = x.reshape(-1)[0]
        try:
            return int(x.item())
        except Exception:
            return int(x)
    if isinstance(x, (list, tuple)):
        return int(x[0])
    try:
        return int(x)
    except Exception:
        return int(default)

from configs.variant_manager import (
    apply_variant_to_yaml,
    get_wrapper_kwargs,
    get_robot_uids,
    get_tool_modes,
)

def _main(args, proc_id: int = 0, start_seed: int = 0) -> str:

    with open(args.config, "r") as f:
        base_cfg = yaml.safe_load(f)

    if args.env_id == "":
        env_id = base_cfg["task_name"] + "-rf"
    else:
        env_id = args.env_id

    raw_task_name = base_cfg["task_name"]
    task_name = _normalize_task_name(raw_task_name)

    final_cfg = apply_variant_to_yaml(
        base_cfg,
        task_name.lower(),
        args.variant,
    )

    robot_uids = get_robot_uids(task_name.lower(), args.variant)
    wrapper_kwargs = get_wrapper_kwargs(task_name.lower(), args.variant)
    tool_modes_raw = get_tool_modes(task_name.lower(), args.variant)

    if isinstance(tool_modes_raw, (list, tuple)) and len(tool_modes_raw) == 1:
        tool_modes_for_solver = tool_modes_raw[0]
    else:
        tool_modes_for_solver = tool_modes_raw
    tmp_cfg = osp.join(args.record_dir, "task_tmp.yaml")
    with open(tmp_cfg, "w") as f_tmp:
        yaml.safe_dump(final_cfg, f_tmp)
    args.config = tmp_cfg

    env = gym.make(
        env_id,
        config=args.config,
        obs_mode=args.obs_mode,
        control_mode=args.control_mode,
        robot_uids=robot_uids,
        render_mode=args.render_mode,
        reward_mode="dense" if args.reward_mode is None else args.reward_mode,
        sensor_configs=dict(shader_pack=args.shader),
        human_render_camera_configs=dict(shader_pack=args.shader),
        viewer_camera_configs=dict(shader_pack=args.shader),
        sim_backend=args.sim_backend,
    )

    env = SuctionActionWrapper(
        env,
        **wrapper_kwargs,
    )

    if not args.traj_name:
        new_traj_name = time.strftime("%Y%m%d_%H%M%S")
    else:
        new_traj_name = args.traj_name
    if args.num_procs > 1:
        new_traj_name = new_traj_name + "." + str(proc_id)

    env = RecordEpisodeMA(
        env,
        output_dir=osp.join(args.record_dir, env_id, "motionplanning"),
        trajectory_name=new_traj_name, save_video=args.save_video,
        source_type="motionplanning",
        source_desc="motion planning solution from RoboFactory",
        video_fps=30,
        save_on_reset=False,
        record_reward=False,
        record_env_state=False,
        record_observation=True,
    )
    output_h5_path = env._h5_file.filename

    solve = MP_SOLUTIONS[env_id]
    print(f"Motion Planning Running on {env_id} with variant={args.variant}")

    pbar = tqdm(range(args.num_traj), desc=f"proc_id: {proc_id}")
    seed = start_seed if args.seed is None else args.seed
    successes = []
    solution_episode_lengths = []
    failed_motion_plans = 0
    passed = 0

    while True:
        #try:
        res = solve(env, seed=seed, debug=False, vis=True if args.vis else False, tool_modes=tool_modes_for_solver)
        #except Exception as e:
        #   print(f"Cannot find valid solution because of an error in motion planning solution: {e}")
        #    res = -1
        if res == -1:
            success = False
            failed_motion_plans += 1
        else:
            # res: tuple(..., info) | info(dict/bool)
            info = res[-1] if isinstance(res, tuple) else res
            if isinstance(info, dict):
                success = _as_py_bool(info.get("success", False))
                elapsed_steps = _as_py_int(info.get("elapsed_steps", 0))
            else:
                success = _as_py_bool(info)
                elapsed_steps = 0
            solution_episode_lengths.append(elapsed_steps)
        successes.append(success)
        if args.only_count_success and not success:
            seed += 1
            env.flush_trajectory(save=False)
            if args.save_video:
                env.flush_video(save=False)
            continue
        else:
            env.flush_trajectory()
            if args.save_video:
                env.flush_video(split=False, split_width=3840)
            pbar.update(1)
            pbar.set_postfix(
                dict(
                    success_rate=np.mean(successes),
                    failed_motion_plan_rate=failed_motion_plans / (seed + 1),
                    avg_episode_length=np.mean(solution_episode_lengths) if solution_episode_lengths else 0.0,
                    max_episode_length=np.max(solution_episode_lengths) if solution_episode_lengths else 0,
                    min_episode_length=np.min(solution_episode_lengths) if solution_episode_lengths else 0
                )
            )
            seed += 1
            passed += 1
            if passed == args.num_traj:
                break
    env.close()
    return output_h5_path

def main(args):
    if args.num_procs > 1 and args.num_procs < args.num_traj:
        if args.num_traj < args.num_procs:
            raise ValueError("Number of trajectories should be greater than or equal to number of processes")
        args.num_traj = args.num_traj // args.num_procs
        seeds = [*range(0, args.num_procs * args.num_traj, args.num_traj)]
        pool = mp.Pool(args.num_procs)
        proc_args = [(deepcopy(args), i, seeds[i]) for i in range(args.num_procs)]
        res = pool.starmap(_main, proc_args)
        pool.close()
        # Merge trajectory files
        output_path = res[0][: -len("0.h5")] + "h5"
        merge_trajectories(output_path, res)
        for h5_path in res:
            tqdm.write(f"Remove {h5_path}")
            os.remove(h5_path)
            json_path = h5_path.replace(".h5", ".json")
            tqdm.write(f"Remove {json_path}")
            os.remove(json_path)
    else:
        _main(args)

if __name__ == "__main__":
    # start = time.time()
    mp.set_start_method("spawn")
    main(parse_args())
    # print(f"Total time taken: {time.time() - start}")
