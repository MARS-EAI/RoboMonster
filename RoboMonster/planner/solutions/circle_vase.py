...
from tasks import CircleVaseEnv
from planner.motionplanner import PandaArmMotionPlanningSolver

def solve(env: CircleVaseEnv, seed=None, debug=False, vis=False, tool_modes=None):
    env.reset(seed=seed)
    ...
    if tool_modes == 'circle':
        ...
        tool.clear_filters()
        tool.allow_only_names(contains=["cube", "card", "vase"])       
        tool.disallow_names(contains=["table", "floor", "ground"])

        planner.open_circle(agent_id=0)
        pose1 = planner.get_grasp_pose_w_labeled_direction(actor=env.vase, actor_data=env.annotation_data['vase'], pre_dis=0, id=0)
        pose1[0] += 0.02
        pose1[2] += 0.2
