    ...
    def get_action(self, policy: BaseImagePolicy, observaton=None):
        ...
        with torch.no_grad():
            ...
            for key in obs_dict.keys():
                if key.startswith('head_cam') or key.startswith('pointcloud') or key.startswith('agent_pos'):          
        ...
        self.cnt += 1
        # # device_transfer
        # action = {k: v.squeeze(0) for k, v in action_dict.items() if not k.startswith('action_pred')}
        # action = dict_apply(action, lambda x: x.detach().to('cpu').numpy())
        # return action
        ...
        split_indices = list(range(8, action.shape[-1], 8))
        action = np.split(action, split_indices, axis=-1)

        action_final = {}
        for agent_id in range(len(action)):
            action_final.update({f'action_{agent_id}': action[agent_id]})
