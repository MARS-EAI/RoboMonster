...
@register_env("SwipeCard-rf", max_episode_steps=500)
class SwipeCardEnv(BaseEnv):
    ...
    def evaluate(self):
        success = self.terminal.pose.p[..., 0] - self.card.pose.p[..., 0] < 0.1 and self.card.pose.p[..., 2] - self.terminal.pose.p[..., 2] < 0.1 and abs(self.terminal.pose.p[..., 1] - self.card.pose.p[..., 1]) < 0.05
        return {
            "success": success,
        }
