...
@register_env("SuctionCard-rf", max_episode_steps=500)
class SuctionCardEnv(BaseEnv):
    SUPPORTED_ROBOTS = ["panda_wristcam", "panda", "fetch", "xarm6_robotiq"]
    ...
    def evaluate(self):
        ...
        is_card_high_enough = self.card.pose.p[..., 2] >= 0.35
        success = is_card_high_enough
        # print("Card Height:", self.card.pose.p[..., 2])
        # print(f"[Eval] is_card_high_enough: {is_card_high_enough}, success: {success}")
        return {
            "is_card_high_enough": is_card_high_enough,
