...
@register_env("PickPokeball-rf", max_episode_steps=500)
class PickPokeballEnv(BaseEnv):
    ...
    def evaluate(self):
        success = self.pokeball.pose.p[..., 2] > 0.2 \
        and abs(self.pokeball.pose.p[..., 1] - self.agent.robot.pose.p[0, 1] - 0.26) < 0.15 \
        and abs(self.pokeball.pose.p[..., 0] - self.agent.robot.pose.p[0, 0] - 0.6) < 0.15
        # print(self.agent.robot.pose.p[0])
        # print(self.pokeball.pose.p)
        # print(abs(self.pokeball.pose.p[..., 1] - self.agent.robot.pose.p[0, 1] - 0.26))
        # print(abs(self.pokeball.pose.p[..., 0] - self.agent.robot.pose.p[0, 0] - 0.6))
        return {
